...
class ChessopiaServer {
    constructor() {
        ...
        // Apply saved per-biome tree chances before initial generation
        try {
            if (fs.existsSync(this.parameterDefaultsPath)) {
                const defaults = JSON.parse(fs.readFileSync(this.parameterDefaultsPath, 'utf8'));
                const biomeNames = ['deep_water','shallow_water','beach','lowland','grassland','forest','mountain','snow'];
                for (let i = 0; i < 8; i++) {
                    const key = `biomeTreeChance${i}`;
                    if (defaults[key] !== undefined && Number.isFinite(defaults[key])) {
                        this.terrainGenerator.biomeTreeChances[biomeNames[i]] = defaults[key];
                    }
                }
                if (defaults.treeMinSlope !== undefined && Number.isFinite(defaults.treeMinSlope)) {
                    this.terrainGenerator.treeMinSlope = defaults.treeMinSlope;
                }
                if (defaults.flatTreeDensity !== undefined && Number.isFinite(defaults.flatTreeDensity)) {
                    this.terrainGenerator.flatTreeDensity = defaults.flatTreeDensity;
                }
                console.log('[Server] Loaded biomeTreeChances from defaults:', JSON.stringify(this.terrainGenerator.biomeTreeChances));
            }
        } catch (e) { /* ignore */ }
        this.terrainGenerator.generateTrees(200); // Generate trees for consistent blocking
        ...
        // Track explicit regeneration vs normal restart
    ...
    setupRoutes() {
        ...
        // Endpoint for setting per-biome tree chances (dev tool, coordinates between clients)
        this.app.post('/api/biome/tree-chances', (req, res) => {
            const chances = req.body?.chances;
            const minSlope = req.body?.minSlope;
            const flatTreeDensity = req.body?.flatTreeDensity;

            if (chances && typeof chances === 'object') {
                console.log('[Server] POST /api/biome/tree-chances received:', JSON.stringify(chances));
                for (const [biome, v] of Object.entries(chances)) {
                    if (!Number.isFinite(v) || v < 0 || v > 10) {
                        console.log(`[Server] Skipped invalid chance for ${biome}: ${v}`);
                        continue;
                    }
                    this.terrainGenerator.biomeTreeChances[biome] = v;
                }
            }

            if (Number.isFinite(minSlope) && minSlope >= 0 && minSlope <= 10) {
                this.terrainGenerator.treeMinSlope = minSlope;
                console.log(`[Server] treeMinSlope updated to ${minSlope}`);
            }

            if (Number.isFinite(flatTreeDensity) && flatTreeDensity >= 0 && flatTreeDensity <= 1) {
                this.terrainGenerator.flatTreeDensity = flatTreeDensity;
                console.log(`[Server] flatTreeDensity updated to ${flatTreeDensity}`);
            }

            console.log(`[Server] Tree chances updated for future generation`);
            console.log('[Server] Final biomeTreeChances:', JSON.stringify(this.terrainGenerator.biomeTreeChances));
            this.io.emit('treeChancesChanged', { chances: this.terrainGenerator.biomeTreeChances });
            res.json({ success: true, chances: this.terrainGenerator.biomeTreeChances, treeCount: this.terrainGenerator.trees.size });
        });
        ...
        this.app.get('/api/terrain/chunk/:chunkX/:chunkZ', (req, res) => {
            ...
            // Check worldData persistent cache (survives server restarts)
            if (this.worldData && this.worldData.chunks && this.worldData.chunks[chunkKey]) {
                console.log(`[Server] Chunk ${chunkKey} restored from persisted world data`);
                const persistedChunk = this.worldData.chunks[chunkKey];
                cache.set(chunkKey, persistedChunk);
                return res.json(persistedChunk);
            }
            ...
            // Persist to worldData for survival across restarts
    ...
    setupSocketHandlers() {
        ...
        this.io.on('connection', (socket) => {
            ...
            // Handle terrain test results sent back from the client
            socket.on('terrainTestResults', (data) => {
                const { passed, failed, total, failures } = data;
                const status = failed === 0 ? '✓ ALL PASS' : `✗ ${failed} FAILED`;
                console.log(`\n[TerrainTests] ${status}  (${passed}/${total} passed)`);
            ...
            // Handle requests for structured client diagnostics from any client or admin tool
            socket.on('requestClientDiagnostics', (request = {}) => {
                console.log(`[Server] Client diagnostics request from ${socket.id}, broadcasting to all clients`);
                this.io.emit('requestClientDiagnostics', request);
            });
            ...
            socket.on('consoleLogs', (logsData) => {
                ...
                if (logsData.meta) {
                    console.log(`Meta: ${JSON.stringify(logsData.meta)}`);
                }

                if (logsData.diagnostics) {
                    const d = logsData.diagnostics;
                    const load = d.load || {};
                    const terrain = d.terrain || {};
                    const board = d.board || {};
                    const network = d.network || {};
                    const consoleStats = d.console?.stats || {};

                    console.log(`--- Diagnostics Snapshot (${d.reason || 'unknown'}) ---`);
                    console.log(`Load: progress=${load.progress ?? '?'}% duration=${load.durationMs != null ? `${load.durationMs.toFixed?.(1) ?? load.durationMs}ms` : 'n/a'} phases=${Array.isArray(load.phases) ? load.phases.length : 0}`);
                    if (terrain) {
                        console.log(`Terrain: chunkSize=${terrain.chunkSize ?? 'n/a'} loadDistance=${terrain.loadDistance ?? 'n/a'} chunksLoaded=${terrain.chunksLoaded ?? 'n/a'} worldDownloaded=${terrain.worldDownloaded ?? 'n/a'}`);
                        console.log(`Terrain Cursor: lastCameraChunk=${terrain.lastCameraChunk ? `${terrain.lastCameraChunk.x},${terrain.lastCameraChunk.z}` : 'n/a'} clientId=${terrain.clientId ?? 'n/a'} pendingDeltas=${terrain.pendingChunkDeltas ?? 'n/a'}`);
                    }
                    if (board) {
                        console.log(`Board: mode=${board.meshMode ?? 'n/a'} viewport=${board.useViewportMesh ?? 'n/a'} continuous=${board.continuousMesh ?? 'n/a'} meshSize=${board.meshBounds?.size ?? 'n/a'} renderDistance=${board.renderDistance ?? 'n/a'}`);
                    }
                    if (network) {
                        console.log(`Network: connected=${network.connected ?? 'n/a'} socketId=${network.socketId ?? 'n/a'} transport=${network.transport ?? 'n/a'} reconnectAttempts=${network.reconnectAttempts ?? 'n/a'}`);
                    }
                    if (d.performance) {
                        console.log(`Performance: FPS=${d.performance.fps ?? 'n/a'} target=${d.performance.targetFps ?? 'n/a'} quality=${d.performance.qualityLevel ?? 'n/a'} vertexBudget=${d.performance.vertexBudget ?? 'n/a'}`);
                    }
                    if (d.weather) {
                        console.log(`Weather: state=${d.weather.weatherState ?? 'n/a'} pressure=${d.weather.pressure ?? 'n/a'} humidity=${d.weather.humidity ?? 'n/a'} temperature=${d.weather.temperature ?? 'n/a'} wind=${d.weather.windSpeed ?? 'n/a'}`);
                    }
                    if (consoleStats) {
                        console.log(`Console: entries=${consoleStats.totalEntries ?? 'n/a'} logs=${consoleStats.byLevel?.log ?? 'n/a'} warns=${consoleStats.byLevel?.warn ?? 'n/a'} errors=${consoleStats.byLevel?.error ?? 'n/a'}`);
                    }

                    socket.data.lastDiagnostics = d;
                }
    ...
    async initializeWorld() {
        ...
        if (this.envSimulation && this.climateInference) {
            ...
            // Restore or initialize world data structure
            if (!this._isRegenerating && worldData && worldData.worldData && worldData.worldData.chunks) {
                this.worldData = {
                    seed: this.worldSeed,
                    chunks: worldData.worldData.chunks,
                    worldBounds: {
                        minX: -400,
                        maxX: 400,
                        minZ: -400,
                ...
                console.log(`[Server] Restored ${Object.keys(this.worldData.chunks).length} chunks from disk`);
            } else {
                this.worldData = {
                    seed: this.worldSeed,
                    chunks: {},
            ...
            // Start periodic world data save to disk
            if (!this._worldSaveInterval) {
                this._worldSaveInterval = setInterval(() => this.saveGeneratedWorldData(), 60000);
            }
    ...
    async generateNewWorld() {
        ...
        // Regenerate biome-aware trees for the new seed (preserving saved chances)
        try {
            if (fs.existsSync(this.parameterDefaultsPath)) {
                const defaults = JSON.parse(fs.readFileSync(this.parameterDefaultsPath, 'utf8'));
                const biomeNames = ['deep_water','shallow_water','beach','lowland','grassland','forest','mountain','snow'];
    ...
    setupConsoleCommands() {
        ...
        process.stdin.on('data', (key) => {
            ...
            // Press 't' to run terrain visual tests on all clients
            if (key === 't') {
                console.log('[Server] Emitting runTerrainTests to all clients...');
                this.io.emit('runTerrainTests');
            }
        });
        ...
        console.log('  Press \'t\' to run terrain visual tests on all clients');
    ...
    async startEnvSimulation() {
        ...
        // envAgents broadcast is handled by the tick loop above (every tickIntervalMs)
