...
class EnvironmentalSimulation {
    ...
    // Remove envFields entries close to neutral (0.5) or far from focal point
    // to prevent unbounded growth as agents explore indefinitely.
    _pruneStaleFields() {
        const stale = [];
        const hardCap = Math.max(5000, Math.min(20000, (this.despawnRadius * 2 + 1) ** 2));
        let farCount = 0;
        let neutralCount = 0;

        for (const [key, field] of this.envFields) {
            const [fx, fz] = key.split(',').map(Number);
            const dx = fx - this.focalX;
            const dz = fz - this.focalZ;
            const dist = Math.sqrt(dx * dx + dz * dz);

            // Cull fields far beyond the despawn radius
            if (dist > this.despawnRadius + 32) {
                stale.push(key);
                farCount++;
                continue;
            }

            // Cull near-neutral fields
            const nearNeutral =
        ...
        // If still over hard cap, cull oldest / furthest entries
        if (this.envFields.size - stale.length > hardCap) {
            const entries = [];
            for (const [key, field] of this.envFields) {
                if (stale.includes(key)) continue;
                const [fx, fz] = key.split(',').map(Number);
                const dist = Math.sqrt((fx - this.focalX) ** 2 + (fz - this.focalZ) ** 2);
                entries.push({ key, dist });
            }
            entries.sort((a, b) => b.dist - a.dist);
            const toCull = this.envFields.size - stale.length - hardCap;
            for (let i = 0; i < toCull && i < entries.length; i++) {
                stale.push(entries[i].key);
            }
        }
        ...
        if (stale.length > 0) {
            console.log(`[EnvSim] Pruned ${stale.length} field entries (far=${farCount} neutral=${neutralCount} cap=${this.envFields.size > hardCap ? 'yes' : 'no'})`);
        }
    }
