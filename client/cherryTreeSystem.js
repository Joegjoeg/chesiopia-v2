...
class CherryTreeSystem {
    constructor(scene, terrainSystem = null) {
        ...
        this._cellAspect = 1.0;
        this.silhouetteAtlas = new THREE.TextureLoader().load('../Images/cherrytree.png', (tex) => {
            if (tex.image) {
                const cellW = tex.image.width / this.atlasCols;
                const cellH = tex.image.height / this.atlasRows;
                this._cellAspect = (cellW / cellH) || 1.0;
            }
        });
    ...
    addTree(worldX, worldZ, terrainHeight, metadata = {}) {
        ...
        const cellAspect = this._cellAspect || 1.0;
        const planeWidth = 2.0;
        const planeHeight = 2.4;
        const targetWidth = planeHeight * cellAspect;
        const widthScale = targetWidth / planeWidth;
