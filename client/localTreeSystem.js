class LocalTreeSystem {
    ...
    updateLOD(cameraPosition, camera) {
        ...
        // Build frustum once per frame without cloning the camera
        ...
        for (const [key, tree] of this.trees) {
            ...
            if (!frustum.containsPoint(tree.position)) {
                tree.visible = false;
                culledFrustum++;
                continue;
            }
    ...
    processChunk(chunkX, chunkZ) {
        ...
        for (let x = 0; x < this.chunkSize; x++) {
            for (let z = 0; z < this.chunkSize; z++) {
                ...
                if (!this.isWithinFogDistance(wx, wz)) {
    ...
    updateTreesAroundCamera() {
        ...
        // Remove trees outside range
        const toRemove = [];
        for (const [key, tree] of this.trees) {
            const [tx, tz] = key.split(',').map(Number);
            if (!this.isWithinFogDistance(tx, tz)) {
                toRemove.push(key);
            }
        }
        for (const key of toRemove) {
            const tree = this.trees.get(key);
            if (tree) this.scene.remove(tree);
            this.trees.delete(key);
        }
