...
class BillboardTreeSystem {
    constructor(scene, terrainSystem = null) {
        ...
        // Width multiplier (slider) to fatten/thin billboard trees independently of image aspect
        this.treeWidthMult = 1.0;
    ...
    async _loadAndBuildAtlas() {
        ...
        const imageFiles = [
            ...
            '../Images/Trees/tree6.jpg'
        ];

        this._variantAspects = [];
        for (let i = 0; i < this.numVariants; i++) {
            const img = await new Promise((resolve, reject) => {
                const image = new Image();
                image.onload = () => resolve(image);
                image.onerror = () => reject(new Error('Failed to load ' + imageFiles[i]));
                image.src = imageFiles[i];
            });

            const aspect = (img.naturalWidth / img.naturalHeight) || 1.0;
            this._variantAspects.push(aspect);

            // Stretch image to fill the entire atlas slice.
            // The instance matrix widthScale will correct the aspect in world space.
            ctx.drawImage(img, i * texSize, 0, texSize, texSize);

            // Make near-white pixels transparent in this atlas slice
            const sliceImgData = ctx.getImageData(i * texSize, 0, texSize, texSize);
            const data = sliceImgData.data;
            ...
            ctx.putImageData(sliceImgData, i * texSize, 0);
        }
        ...
        return new THREE.ShaderMaterial({
            ...
            vertexShader: `
                ...
                void main() {
                    ...
                    float stretchY = mix(uStretchMax, uStretchMin, stretchT);
    ...
    addTree(worldX, worldZ, terrainHeight, metadata = {}) {
        ...
        const aspectRatio = this._variantAspects && this._variantAspects[variant] ? this._variantAspects[variant] : 1.0;
        const planeWidth = 1.8;
        const planeHeight = 2.8;
        const targetWidth = planeHeight * aspectRatio;
        const widthScale = (targetWidth / planeWidth) * this.treeWidthMult;

        for (const part of this.parts) {
            this._scratchPos.set(worldX, terrainHeight, worldZ);
            this._scratchScale.set(scale * widthScale, scale, scale);
    ...
    updateTreeInstanceMatrix(i, tree) {
        const ps = window.parameterSystem;
        const upright = ps ? ps.getParameter('treeUpright') : false;
        const normal = upright ? UP : (tree.normal || UP);
        this._scratchQuat.setFromUnitVectors(UP, normal);
        ...
        for (const part of this.parts) {
            ...
            const s = tree.scale * (this.globalTreeSizeMult || 1.0);
            this._scratchScale.set(s * widthScale, s, s);
            this._scratchMatrix.compose(this._scratchPos, this._scratchQuat, this._scratchScale);
            part.mesh.setMatrixAt(i, this._scratchMatrix);
            part.mesh.instanceMatrix.needsUpdate = true;
        }
    ...
    updateTreeHeights() {
        ...
        for (let i = 0; i < this.treeCount; i++) {
            ...
            const ps = window.parameterSystem;
            const upright = ps ? ps.getParameter('treeUpright') : false;
            if (!upright && board.getTerrainNormal) {
    ...
    setMaxTrees(value) {
        ...
        this.parts.forEach(p => this.scene.add(p.mesh));
        this.clear();
        console.log('[BillboardTreeSystem] Max trees resized to', value);
    }

    setTreeWidthMult(value) {
        if (this.treeWidthMult === value) return;
        this.treeWidthMult = value;
        // Rebuild all instance matrices so existing trees pick up the new width
        for (let i = 0; i < this.treeCount; i++) {
            this.updateTreeInstanceMatrix(i, this.treeData[i]);
        }
        for (const part of this.parts) {
            if (part.mesh) part.mesh.instanceMatrix.needsUpdate = true;
        }
        console.log('[BillboardTreeSystem] treeWidthMult set to', value);
    }

    update(timeSec, windStrength, windDirection) {
        const ps = window.parameterSystem;
        if (ps) {
            const twm = ps.getParameter('treeWidthMult');
            if (typeof twm === 'number' && twm !== this.treeWidthMult) {
                this.setTreeWidthMult(twm);
            }
        }
        ...
        if (bs && bs.sun && bs.moon) {
            ...
            // Use actual scene light direction instead of hardcoded shader value
            const sunDir = bs.sun.light.position.clone().normalize();
            const moonDir = bs.moon.light.position.clone().normalize();
            const blendedDir = new THREE.Vector3()
                .addScaledVector(sunDir, sunInt)
                .addScaledVector(moonDir, moonInt);
            const lightDir = blendedDir.lengthSq() > 0 ? blendedDir.normalize() : new THREE.Vector3(0, 1, 0);

            for (const part of this.parts) {
                if (!part.mesh || !part.mesh.material || !part.mesh.material.uniforms) continue;
                const u = part.mesh.material.uniforms;
                if (u.uSunIntensity) u.uSunIntensity.value = totalInt;
                if (u.ambient) u.ambient.value = amb;
                if (u.lightDir) u.lightDir.value.copy(lightDir);
            }
        }

        // Sync fog uniforms from scene so billboards match terrain/objects
        const scene = this.scene;
        if (scene && scene.fog) {
            for (const part of this.parts) {
                if (!part.mesh || !part.mesh.material || !part.mesh.material.uniforms) continue;
                const u = part.mesh.material.uniforms;
                if (u.fogColor) u.fogColor.value.copy(scene.fog.color);
                if (u.fogNear) u.fogNear.value = scene.fog.near;
                if (u.fogFar) u.fogFar.value = scene.fog.far;
            }
        }
