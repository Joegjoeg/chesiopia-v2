class CameraController {
    constructor(camera, scene) {
        ...
        // Movement
        this.moveSpeed = 0.2; // Aligned with maxSpeed for direct, lag-free WASD control
    ...
    handleMouseDown(event) {
        if (event.button === 0) { // Left click - now for pieces only
            ...
            // Initialize orbit from current camera position relative to currentTarget
            // (camera is placed around currentTarget in updateCameraPosition, not target)
            ...
            // Only recompute azimuth when horizontal offset is well-defined.
            // Near-vertical views have tiny horizontalDist, making atan2 unstable.
    ...
    updateStrategicMode() {
        ...
        if (moveVector.lengthSq() > 0) {
            ...
            // Direct control: eliminate velocity lag so keys start/stop immediately
            this.currentTarget.copy(this.target);
            this.velocity.set(0, 0, 0);
        }
    ...
    updateTacticalMode() {
        // W/S move forward/back along camera direction; A/D rotate around target.
        const moveVector = new THREE.Vector3();
        ...
        if (rotated) {
            // Keep target/currentTarget in sync so drag transitions stay smooth
            this.currentTarget.copy(this.target);
            this.velocity.set(0, 0, 0);
        }
