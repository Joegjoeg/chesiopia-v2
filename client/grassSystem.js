...
class GrassSystem {
    ...
    // Sample dominant Gerstner harmonic from rolling terrain for unified wind/wave physics.
    // Returns { kx, kz, omega, phaseOffset, ampScale } or null if unavailable.
    _getWaveParams() {
        const rt = this.boardSystem?.rollingTerrain;
        if (!rt || !rt.waveConfig?.enabled || !rt.waveConfig.harmonics?.length) {
            return null;
        }
        const h = rt.waveConfig.harmonics[0];
        const L = rt.N * rt.S;
        if (!L) return null;
        const g = 9.81;
        const kx = (2 * Math.PI * h.nx) / L;
        const kz = (2 * Math.PI * h.nz) / L;
        const kMag = Math.sqrt(kx * kx + kz * kz) || 0.001;
        const omega = Math.sqrt(g * kMag) * rt.waveConfig.speed * (1.0 + rt.windSpeed * 0.2);
        const windPhaseBoost = rt.windSpeed * 0.3;
        const windDx = rt.windDir.x;
        const windDz = rt.windDir.y;
        const windDot = (kx * windDx + kz * windDz) / kMag;
        const phaseOffset = h.phase + windPhaseBoost * windDot;
        const ampScale = h.amplitude * rt.waveConfig.amplitudeScale;
        return { kx, kz, omega, phaseOffset, ampScale };
    }
    ...
    update(deltaTime, cameraPosition) {
        ...
        // Try to sync with water wave time for perfect coherence
        const rt = this.boardSystem?.rollingTerrain;
        const waveTime = (rt && rt.waterUniforms?.uTime?.value != null)
            ? rt.waterUniforms.uTime.value
            : this.animationTime;

        const waveParams = this._getWaveParams();

        // Unified wind direction from rolling terrain (falls back to minimap)
        const localWind = window.minimapOverlay?.localWind;
        let windAngle;
        if (rt && rt.windDir) {
            windAngle = Math.atan2(rt.windDir.y, rt.windDir.x);
        } else if (localWind) {
            windAngle = localWind.angle;
        } else {
            windAngle = 0;
        }
        ...
        for (const [chunkKey, grassData] of this.grassChunks) {
            ...
            for (let i = 0; i < basePositions.length; i++) {
                ...
                let sway;
                if (waveParams) {
                    // Gerstner-derived wave phase — spatially coherent with water
                    const worldPhase = waveParams.kx * basePos.x + waveParams.kz * basePos.z;
                    const wavePhase = worldPhase - waveParams.omega * waveTime + waveParams.phaseOffset;
                    sway = Math.sin(wavePhase) * strength * waveParams.ampScale * 2.0;
                } else {
                    // Fallback to legacy hardcoded sway
                    const phase = (basePos.x * 0.3 + basePos.z * 0.7) * Math.PI * 2 * phaseScl;
                    sway = Math.sin(this.animationTime * 2.0 + phase) * strength;
                }

                // Orient sway + static push in unified wind direction
