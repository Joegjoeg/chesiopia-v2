...
class ModifierStack {
  ...
  toLegacyUniforms() {
    ...
    return {
      uBiomeEdgeScale: wiggle ? wiggle.params.scale : 0.3,
      uBiomeEdgeStrength: wiggle ? wiggle.params.strength : 1.0,
      uBiomeSplatterScale: splatter ? splatter.params.scale : 0.5,
      uBiomeSplatterAmount: splatter ? splatter.params.strength : 0.5,
      uBiomeEdgeSplatterMix: splatter ? (splatter.blendOp === 'mix' ? 0.5 : 0.0) : 0.0,
      uBiomeEdgeA: wiggle ? wiggle.targetBiomeLow : 3,
      uBiomeEdgeB: wiggle ? wiggle.targetBiomeHigh : 4,
      uBiomeEdgeMode: active.length > 0 ? 2 : 0
    };
  }
