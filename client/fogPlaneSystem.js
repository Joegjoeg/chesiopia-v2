...
class FogPlaneSystem {
    ...
    setWeatherSnapshot(snapshot) {
        if (!snapshot) return;
        this.envPressure = snapshot.pressure;
        this.envHumidity = snapshot.humidity;
        this.envTemperature = snapshot.temperature;
        this.updateInstantaneousTemperature();
        this.setWind(snapshot.windSpeed, snapshot.windDirection);
        if (this.material) {
            this.material.uniforms.uHumidity.value = snapshot.humidity;
            this.material.uniforms.uPressure.value = snapshot.pressure;
            const ps = window.parameterSystem;
            const fogScale = ps ? ps.getParameter('weatherFogDensityScale') : 1.0;
            this.material.uniforms.uFogDensity.value = snapshot.fogDensity * fogScale;
        }
    }
    ...
    setEnabled(enabled) {
        ...
        if (this.material) {
            this.material.uniforms.uEnabled.value = enabled ? 1.0 : 0.0;
        }
        if (this.mesh) {
    ...
    _buildShader() {
        ...
        const fragmentShader = `
            ...
            void main() {
                ...
                // Environmental density from WeatherDirector (single source of truth)
                float envFog = uFogDensity;
