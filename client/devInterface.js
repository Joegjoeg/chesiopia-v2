...
class DevInterface {
    constructor() {
        ...
        this.categories = ['terrain', 'planet', 'lighting', 'spotlight', 'time', 'environment', 'graphics', 'taa', 'performance', 'lod', 'distances', 'water', 'reflection', 'shoreline', 'landCover', 'cliff', 'tree', 'blending', 'verts', 'camera', 'sky', 'stars', 'rig', 'checkerboard', 'models', 'jesus', 'settlement', 'flare', 'minimap', 'cursor', 'shader', 'weather', 'bridge'];
    ...
    setupEventListeners() {
        if (this.parameterSystem && this.parameterSystem.onParameterChange) {
            this._parameterChangeUnsub = this.parameterSystem.onParameterChange((name, value) => {
                ...
                if (name.startsWith('biomeColor') || name.startsWith('biomeThreshold') || name.startsWith('biomeEdge') || name.startsWith('biomeSplatter') || name.startsWith('biomePatch') || name.startsWith('grassType') || name.startsWith('biomeTreeChance') || name.startsWith('biomeTexLayer') || name.startsWith('biomeTexScale') || name.startsWith('biomeTexNoiseScale')) {
                    if (this._biomeListContainer) this._rebuildBiomeList();
                    if (this._biomeDetailPanel) this._rebuildBiomeDetail();
                }
            });
        }
    }
    ...
    createInterface() {
        ...
        const categoryLabels = { shoreline: 'Shore', landCover: 'Land', graphics: 'GRA', taa: 'TAA', tree: 'TRE', blending: 'BLD', verts: 'GEO', camera: 'CAM', rig: 'RIG', checkerboard: 'CHK', models: 'MDL', jesus: 'JES', settlement: 'SET', distances: 'DST', flare: 'FLR', minimap: 'MAP', spotlight: 'SPT', reflection: 'REF', shader: 'SHD', weather: 'WTH', bridge: 'BRG' };
    ...
    createCategoryContent(category) {
        ...
        if (category === 'bridge') {
            return this._createBridgeContent();
        }
    ...
    _buildParameterSection(category) {
        ...
        Object.entries(parameters).forEach(([name, config]) => {
            ...
            if (config.type === 'header') {
                const header = document.createElement('div');
                header.textContent = config.shortLabel || config.description || name;
                header.style.cssText = `
                    font-size: 9px;
                    font-weight: 700;
                    color: #00ff99;
                    ...
                    margin-top: 2px;
                `;
                paramRow.appendChild(header);
                paramsContainer.appendChild(paramRow);
                return; // skip reset button and further handling
                ...
                slider.title = config.description || config.shortLabel || name;
    ...
    _postDefaultsToServer(config, fileName) {
        // Diff against current parameter defaults to only send overrides
        const overrides = {};
        Object.entries(config).forEach(([name, val]) => {
            const param = this.parameterSystem.params.get(name);
            if (param && param.persist !== false) {
        ...
        const payload = Object.keys(overrides).length > 0 ? overrides : config;
    ...
    _startTaaStatusPolling() {
        // Stub — TAA panel doesn't need live polling yet
    }
    ...
    _createBiomeContent() {
        ...
        resetBtn.title = 'Reset selected biome to default';
        resetBtn.onclick = () => this._biomeReset();
        toolbar.appendChild(resetBtn);
        const resetAllBtn = document.createElement('button');
        resetAllBtn.textContent = 'Reset All';
        resetAllBtn.style.cssText = btnStyle;
        resetAllBtn.title = 'Reset all biomes to defaults';
        resetAllBtn.onclick = () => this._biomeResetAll();
        toolbar.appendChild(resetAllBtn);

        // Global texture blend slider
        const blendLabel = document.createElement('span');
        blendLabel.textContent = 'TexBlend';
        blendLabel.style.cssText = `font-size: 8px; color: #aaffaa; margin-left: 4px;`;
        ...
        blendSlider.value = this.parameterSystem.getParameter('biomeTextureBlend') ?? 0.7;
        blendSlider.style.cssText = `width: 60px; height: 4px; cursor: pointer; accent-color: #00ff99;`;
        blendSlider.title = 'Global PBR texture blend strength';
        blendSlider.addEventListener('input', (e) => {
            this.parameterSystem.setParameter('biomeTextureBlend', parseFloat(e.target.value));
        });
        toolbar.appendChild(blendSlider);
    ...
    _biomeResetAll() {
        for (let i = 0; i < 8; i++) {
            this.parameterSystem.resetParameter(`biomeColor${i}`);
            if (i > 0) this.parameterSystem.resetParameter(`biomeThreshold${i-1}`);
        }
        this._biomeNames = ['Deep Water','Shallow Water','Beach','Lowland','Grassland','Forest','Mountain','Snow'];
        this._rebuildBiomeList();
        this._rebuildBiomeDetail();
        this._showToast('All biomes reset to defaults');
    }
    ...
    _rebuildBiomeDetail() {
        ...
        const mkDropdown = (label, paramKey, includeNone) => {
            ...
            sel.style.cssText = `flex:1; background: rgba(0,0,0,0.4); border: 1px solid rgba(0,255,0,0.2); color: #00ff00; padding: 1px 3px; border-radius: 2px; font-size: 9px;`;
            if (includeNone) {
                const noneOpt = document.createElement('option');
                noneOpt.value = noneOption.value;
                noneOpt.textContent = noneOption.label;
                sel.appendChild(noneOpt);
            }
            textureSetNames.forEach(name => {
                const opt = document.createElement('option');
                opt.value = name;
                opt.textContent = name;
                sel.appendChild(opt);
            });
            sel.value = this.parameterSystem.getParameter(paramKey) || '';
            sel.addEventListener('change', (e) => {
                this.parameterSystem.setParameter(paramKey, e.target.value);
            });
            row.appendChild(sel);
            return row;
        };
        const mkScaleSlider = (label, paramKey) => {
            const row = document.createElement('div');
            row.style.cssText = `display: flex; align-items: center; gap: 6px;`;
            const lbl = document.createElement('span');
            lbl.textContent = label;
            ...
            slider.max = 5.0;
            slider.step = 0.01;
            const val = this.parameterSystem.getParameter(paramKey) ?? 1.0;
            slider.value = val;
            slider.style.cssText = `flex:1; height: 4px; cursor: pointer; accent-color: #00ff99;`;
            const disp = document.createElement('span');
            disp.textContent = parseFloat(val).toFixed(2) + 'x';
            disp.style.cssText = `font-size: 9px; color: #00ff00; width: 30px; text-align: right;`;
            slider.addEventListener('input', (e) => {
                const v = parseFloat(e.target.value);
                disp.textContent = v.toFixed(2) + 'x';
                this.parameterSystem.setParameter(paramKey, v);
            });
        ...
        // Layer 0
        const layer0Title = document.createElement('div');
        layer0Title.textContent = 'Layer 0 (Base)';
        layer0Title.style.cssText = `font-size: 9px; font-weight: 600; color: #aaffaa; margin-top: 4px; border-bottom: 1px solid rgba(0,255,0,0.08); padding-bottom: 2px;`;
        panel.appendChild(layer0Title);
        panel.appendChild(mkDropdown('Texture', `biomeTexLayer0_${i}`, false));
        panel.appendChild(mkScaleSlider('Scale', `biomeTexScale0_${i}`));

        // Layer 1
        const layer1Title = document.createElement('div');
        layer1Title.textContent = 'Layer 1 (Overlay)';
        layer1Title.style.cssText = `font-size: 9px; font-weight: 600; color: #aaffaa; margin-top: 4px; border-bottom: 1px solid rgba(0,255,0,0.08); padding-bottom: 2px;`;
        panel.appendChild(layer1Title);
        panel.appendChild(mkDropdown('Texture', `biomeTexLayer1_${i}`, true));
        panel.appendChild(mkScaleSlider('Scale', `biomeTexScale1_${i}`));
        panel.appendChild(mkScaleSlider('Noise Sc', `biomeTexNoiseScale_${i}`));
        ...
        // Tree chance slider (per-biome, server-synced)
        const treeChanceRow = document.createElement('div');
        treeChanceRow.style.cssText = `display: flex; align-items: center; gap: 6px;`;
        const treeChanceLabel = document.createElement('span');
        treeChanceLabel.textContent = 'Trees';
        treeChanceLabel.style.cssText = `font-size: 9px; color: #aaffaa; width: 50px;`;
        treeChanceRow.appendChild(treeChanceLabel);
        const treeChanceSlider = document.createElement('input');
        treeChanceSlider.type = 'range';
        treeChanceSlider.min = 0;
        treeChanceSlider.max = 1;
        treeChanceSlider.step = 0.05;
        treeChanceSlider.value = this.parameterSystem.getParameter(`biomeTreeChance${i}`) ?? 1.0;
        treeChanceSlider.style.cssText = `flex:1; height: 4px; cursor: pointer; accent-color: #00ff99;`;
        treeChanceSlider.title = 'Tree spawn chance for this biome';
        const treeChanceDisplay = document.createElement('span');
        treeChanceDisplay.textContent = Math.round((treeChanceSlider.value) * 100) + '%';
        treeChanceDisplay.style.cssText = `font-size: 9px; color: #00ff00; width: 30px; text-align: right;`;
        treeChanceSlider.addEventListener('change', (e) => {
            const v = parseFloat(e.target.value);
            treeChanceDisplay.textContent = Math.round(v * 100) + '%';
            this.parameterSystem.setParameter(`biomeTreeChance${i}`, v, 'user');
        });
        treeChanceRow.appendChild(treeChanceSlider);
        treeChanceRow.appendChild(treeChanceDisplay);
        panel.appendChild(treeChanceRow);
        // Start height
        if (i > 0) {
        ...
        if (i < 7) {
            ...
            const modeOpts = [
                { value: 0, label: 'Blended (smooth transition)' },
                { value: 1, label: 'Sharp (hard cut)' },
                { value: 2, label: 'Custom (noise + splatter)' }
            ];
            const curMode = this.parameterSystem.getParameter(`biomeEdgeMode${i}`);
            modeOpts.forEach(o => {
                const opt = document.createElement('option');
            ...
            modeSelect.addEventListener('change', (e) => {
                this.parameterSystem.setParameter(`biomeEdgeMode${i}`, parseInt(e.target.value));
            });
            ...
            panel.appendChild(mkParam(`biomeEdgeScale${i}`, 'Wiggle Sc'));
            panel.appendChild(mkParam(`biomeEdgeStrength${i}`, 'Wiggle Str'));
            panel.appendChild(mkParam(`biomeSplatterScale${i}`, 'Splat Sc'));
            panel.appendChild(mkParam(`biomeSplatterAmount${i}`, 'Splat Amt'));
            panel.appendChild(mkParam(`biomeEdgeSplatterMix${i}`, 'Edge/Splat'));
        }

        // Global patch noise
        const patchTitle = document.createElement('div');
        patchTitle.textContent = 'Global Patch Noise';
        patchTitle.style.cssText = `font-size: 9px; font-weight: 600; color: #aaffaa; margin-top: 4px; border-bottom: 1px solid rgba(0,255,0,0.08); padding-bottom: 2px;`;
        panel.appendChild(patchTitle);
        const mkGlobal = (paramName, label) => {
            const row = document.createElement('div');
            row.style.cssText = `display: flex; align-items: center; gap: 6px;`;
            const lbl = document.createElement('span');
            lbl.textContent = label;
        ...
        panel.appendChild(mkGlobal('biomePatchScale', 'Scale'));
        panel.appendChild(mkGlobal('biomePatchStrength', 'Strength'));
        panel.appendChild(mkGlobal('biomePatchSeed', 'Seed'));

        // Modifier Stack summary
        const stack = this.parameterSystem.getParameter('biomeModifierStack');
        if (stack) {
            const modTitle = document.createElement('div');
            modTitle.textContent = 'Modifier Stack';
            modTitle.style.cssText = `font-size: 9px; font-weight: 600; color: #aaffaa; margin-top: 4px; border-bottom: 1px solid rgba(0,255,0,0.08); padding-bottom: 2px;`;
            panel.appendChild(modTitle);
            const modRow = document.createElement('div');
            modRow.style.cssText = `display: flex; align-items: center; gap: 6px;`;
            const modLbl = document.createElement('span');
            modLbl.textContent = 'Layers';
            modLbl.style.cssText = `font-size: 9px; color: #aaffaa; width: 50px;`;
            modRow.appendChild(modLbl);
            const modCount = document.createElement('span');
            modCount.textContent = String(stack.layers ? stack.layers.length : 0);
            modCount.style.cssText = `font-size: 9px; color: #00ff00; flex:1;`;
            modRow.appendChild(modCount);
            const modBtn = document.createElement('button');
            modBtn.textContent = 'Edit';
            modBtn.style.cssText = `background: rgba(0,255,0,0.08); border: 1px solid rgba(0,255,0,0.25); color: #00ff00; padding: 1px 5px; border-radius: 2px; cursor: pointer; font-size: 8px;`;
            modBtn.onclick = () => {
                window.open('test-tools/terrain-node-wrangler/index.html', '_blank');
            };
            modRow.appendChild(modBtn);
    ...
    _createCliffContent() {
        ...
        const makeSlider = (name, label) => {
            ...
            slider.title = cfg.description || label;
    ...
    _createCheckerboardContent() {
        ...
        fadeSlider.title = 'Transparency fade strength for checkerboard pattern';
    ...
    _updateSharedRigEditor(section, rig) {
        ...
        intSlider.title = 'Light intensity at this keyframe';
        ...
        if (kf.transparency !== undefined) {
            ...
            transSlider.title = 'Sky transparency at this keyframe';
    ...
    _createJesusContent() {
        ...
        slider.title = 'Vertical lift height for Jesus summon';
    ...
    _createWeatherContent() {
        ...
        // Cloud debug section
        const cloudDebugHeader = document.createElement('div');
        cloudDebugHeader.textContent = 'CLOUD DEBUG';
        cloudDebugHeader.style.cssText = 'font-size: 9px; font-weight: 600; color: #00ff00; margin-top: 8px; border-top: 1px solid rgba(0,255,0,0.1); padding-top: 4px;';
        section.appendChild(cloudDebugHeader);

        const cloudDebugRow = document.createElement('div');
        cloudDebugRow.style.cssText = 'display: flex; gap: 4px; margin-top: 4px;';

        const spawnCloudBtn = document.createElement('button');
        spawnCloudBtn.textContent = 'Spawn Test Cloud at (0,0)';
        spawnCloudBtn.style.cssText = `
            flex: 1; background: rgba(255,200,0,0.15); border: 1px solid rgba(255,200,0,0.4);
            color: #ffcc00; padding: 5px; border-radius: 3px; cursor: pointer; font-size: 9px;
        `;
        spawnCloudBtn.onclick = () => {
            if (window.game?.voxelCloudSystem) {
                window.game.voxelCloudSystem.spawnTestCloudAt(0, 0);
            }
        };
        cloudDebugRow.appendChild(spawnCloudBtn);

        const clearCloudBtn = document.createElement('button');
        clearCloudBtn.textContent = 'Clear Test';
        clearCloudBtn.style.cssText = `
            flex: 1; background: rgba(255,50,50,0.15); border: 1px solid rgba(255,50,50,0.4);
            color: #ff6666; padding: 5px; border-radius: 3px; cursor: pointer; font-size: 9px;
        `;
        clearCloudBtn.onclick = () => {
            if (window.game?.voxelCloudSystem) {
                window.game.voxelCloudSystem.clearTestClouds();
            }
        };
        cloudDebugRow.appendChild(clearCloudBtn);

        section.appendChild(cloudDebugRow);
    ...
    _createEnvironmentContent() {
        ...
        const subsections = [
            ...
            },
            {
                ...
                names: ['windDebrisEnabled', 'windDebrisDensity', 'windParticleSize', 'windParticleColor', 'windParticleGravity', 'windTwigFrequency', 'windTwigGravity', 'windGravityFactor', 'windSpinSpeed', 'windSoundEnabled', 'windSoundVolume']
            },
            {
                title: 'Fog',
    ...
    _createBridgeContent() {
        ...
        const section = document.createElement('div');
        section.dataset.categorySection = 'bridge';
        section.style.cssText = `
            border: 1px solid rgba(0, 255, 0, 0.15);
            border-radius: 4px;
            padding: 6px 8px;
        ...
        const header = document.createElement('div');
        header.textContent = 'BRIDGE MATERIAL';
        header.style.cssText = `
            font-size: 9px;
            font-weight: 600;
            color: #00ff00;
        ...
        // Brick size slider
        const sizeRow = makeRow('Brick Size');
        const sizeSlider = document.createElement('input');
        sizeSlider.type = 'range';
        sizeSlider.min = 0.1;
        sizeSlider.max = 2.0;
        sizeSlider.step = 0.05;
        sizeSlider.value = settings.brickSize;
        ...
        sizeDisplay.textContent = settings.brickSize.toFixed(2);
        ...
        sizeSlider.addEventListener('input', (e) => {
            ...
            settings.brickSize = val;
            sizeDisplay.textContent = val.toFixed(2);
        });
        sizeRow.row.appendChild(sizeSlider);
        sizeRow.row.appendChild(sizeDisplay);
        section.appendChild(sizeRow.row);

        // Brick colour picker
        const brickRow = makeRow('Brick Colour');
        const brickColor = document.createElement('input');
        brickColor.type = 'color';
        brickColor.value = settings.brickColor;
        brickColor.style.cssText = `
            width: 28px;
            height: 16px;
            border: 1px solid rgba(0, 255, 0, 0.3);
            border-radius: 2px;
            ...
            cursor: pointer;
        `;
        brickColor.addEventListener('input', (e) => {
            settings.brickColor = e.target.value;
        });
        brickRow.row.appendChild(brickColor);
        section.appendChild(brickRow.row);

        // Mortar colour picker
        const mortarRow = makeRow('Mortar Colour');
        const mortarColor = document.createElement('input');
        mortarColor.type = 'color';
        mortarColor.value = settings.mortarColor;
        mortarColor.style.cssText = `
            width: 28px;
            height: 16px;
            border: 1px solid rgba(0, 255, 0, 0.3);
            border-radius: 2px;
            ...
            cursor: pointer;
        `;
        mortarColor.addEventListener('input', (e) => {
            settings.mortarColor = e.target.value;
        });
        mortarRow.row.appendChild(mortarColor);
        section.appendChild(mortarRow.row);
