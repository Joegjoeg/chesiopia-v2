class TerrainGenerator {
    constructor() {
        ...
        this.biomeTreeChances = {
            deep_water: 1.0, shallow_water: 1.0, beach: 1.0,
            lowland: 1.0, grassland: 1.0, forest: 1.0,
            mountain: 1.0, snow: 1.0
        };
        this.treeMinSlope = 0; // Reduction applied to biome minSlope (higher = flatter terrain allowed)
        this.flatTreeDensity = 0.15; // Fraction of biome density used for random flat-terrain fallback
        this._treeDebug = true; // Temporarily enable to diagnose tree generation
        ...
        // Settlement building flattening overrides
        this.heightModifications = new Map();
    ...
    generateTrees(searchRadius = 50) {
        ...
        console.log(`[TerrainGen] Chances:`, JSON.stringify(this.biomeTreeChances));
        console.log(`[TerrainGen] treeMinSlope: ${this.treeMinSlope}`);
        ...
        for (let x = -searchRadius; x <= searchRadius; x++) {
            for (let y = -searchRadius; y <= searchRadius; y++) {
                ...
                if (!this.trees.has(key)) {
                    let reason = this._tryPlaceTree(x, y);
                    if (!this.trees.has(key)) {
                        reason = this._tryPlaceTree(x, y, true);
                    }
                    if (this.trees.has(key)) {
                        // tree added
                    } else if (reason) {
        ...
        console.log(`[TerrainGen] Generated ${this.trees.size} biome-aware trees, rejections:`, reasons);
    }
    ...
    // allowFlat = true adds a low-density fallback for flat terrain that would normally fail slope checks
    _tryPlaceTree(x, y, allowFlat = false) {
        const height = this.getHeight(x, y);
        if (height < this.waterLevel) return 'underwater';

        const slope = this.calculateSlope(x, y, height);
        const biome = this.getBiomeType(height, x, y);
        const config = this.BIOME_TREE_CONFIG[biome];
        if (!config || config.density <= 0) return 'zero-density';

        const effectiveMinSlope = Math.max(0, config.minSlope - (this.treeMinSlope || 0));
        const onSlope = slope >= effectiveMinSlope && slope <= config.maxSlope;

        if (!onSlope) {
            if (!allowFlat) return 'slope';
            // Flat fallback: avoid very steep ground and use a much lower density
            if (slope > 45) return 'slope';
        }

        let density = config.density;
        if (!onSlope && allowFlat) {
            density = config.density * (this.flatTreeDensity || 0.15); // sparse random trees on flat ground
            if (density <= 0) return 'zero-density';
        }

        const rng = this._tileRandom(x, y, 0);
        if (rng > density * (this.biomeTreeChances[biome] ?? 1.0)) return 'rng';

        // Clumpiness: only for slope-based trees
        if (onSlope && config.clumpiness > 0) {
            ...
            if (clumpNoise < clumpThreshold) return 'clump';
        }

        // Per-tree variation (flat trees are slightly smaller on average)
        const scaleVar = this._tileRandom(x, y, 1);
        const maxScale = config.maxScale * (0.8 + scaleVar * 0.4) * (onSlope ? 1.0 : 0.85);
    ...
    generateTreesForChunk(chunkX, chunkZ, chunkSize = 16) {
        ...
        for (let z = 0; z < chunkSize; z++) {
            for (let x = 0; x < chunkSize; x++) {
                ...
                const key = `${wx},${wz}`;
                if (!this.trees.has(key)) {
                    let reason = this._tryPlaceTree(wx, wz);
                    if (!this.trees.has(key)) {
                        reason = this._tryPlaceTree(wx, wz, true);
                    }
                    if (this.trees.has(key)) added++;
                    else if (reason) reasons[reason] = (reasons[reason] || 0) + 1;
                }
            }
        }
        if (this._treeDebug || added > 0) {
            console.log(`[TerrainGen] chunk (${chunkX},${chunkZ}) added=${added} treeMinSlope=${this.treeMinSlope}`, reasons);
        }
        return added;
    }
    ...
    clearCache() {
        ...
        console.log('[TerrainGenerator] Height caches cleared');
    }
    ...
    applyHeightDeltas(chunkKey, deltas) {
        for (const d of deltas) {
            const gx = parseInt(chunkKey.split(',')[0]) * 16 + d.localX;
            const gz = parseInt(chunkKey.split(',')[1]) * 16 + d.localZ;
            this.heightModifications.set(`${gx},${gz}`, d.height);
        }
    }
}
